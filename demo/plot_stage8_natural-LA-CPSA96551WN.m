...
%     ecc    - eccentricity (default 1).
    ...
    pimg = imresize(pimg, 0.5);              % downsample by half: a quarter as many patches,
                                             % so the (partly serial) grouping stage is far cheaper.
                                             % Each 64px patch now spans ~2x the visual angle.
    ...
    fprintf('stage 8: segmenting a %dx%d natural region (%dx%d patches of %dpx)...\n', ...
    ...
    h = imagesc(groups_up); colormap(gca, 'jet'); set(h, 'AlphaData', 0.2);   % faint colour fill
    for lab = unique(groups2d(groups2d > 0))'                                  % outline each region
        B = bwboundaries(groups_up == lab);
        for m = 1:numel(B)
            plot(B{m}(:, 2), B{m}(:, 1), 'w', 'LineWidth', 1.5);
        end
    end
