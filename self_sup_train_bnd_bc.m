...
elseif filter == 1
  name = append('dbndbNO',num,'.mat');  % trained on joined
end
load(name);
fun_level = 0;
...
ncnt = 2*(szp-1)^2;
%
% This next step is for information and not really used
% Make maps, count same and different for near and far patches 
for k = 1:nmaps
...
propsm = smcnt/cnt; 
propdf = dcnt/cnt; % these proportions are information that is not used
% 
...
gcmn = 0; dgc = 0.4; gcmx = 4; ngc = (gcmx-gcmn)/dgc + 1;
%
% mutual similarity weight
msmn = 0; dms = 0.4; msmx = 4; nms = (msmx-msmn)/dms + 1;
%
% storage
...
for trl = 1:ntrl
  ...
  %
  % patch luminance image; this luminance image is not used here
  for i = 1:psz:imw
    for j = 1:psz:imw
  ...
  %
  % plot GTR image or luminance image if desired
  if plotflg == 1
    img(:,:,1) = pimg(:,:,1); % limg(:,:,1)
    img(:,:,2) = pimg(:,:,1); % limg(:,:,2)
    img(:,:,3) = pimg(:,:,1); % limg(:,:,3)    
...
%  
  ...
  %
  % learn decision bound
  if pltbnd == 1
     results_bc = classify_normals([rbn',rcn'],[rbf',rcf'],'input_type',...
      'samp','plotmode',0);
     dbndbc = results_bc.samp_opt_bd;  % sample-optimized bound   
     results_bc=classify_normals([rbn',rcn'],[rbf',rcf'],'input_type',...
         'samp','dom',dbndbc,'samp_opt',0);     
    axis normal
    fontsize(14,'points');
    xlabel Border
    ylabel Content  
  ...
  fun_level = 0;
  dbndbc = results_bc.samp_opt_bd;  % QSVM bound
