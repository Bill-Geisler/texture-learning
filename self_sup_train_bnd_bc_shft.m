...
% (3) learn the optimal shift (offset) of the border-content decision bound
...
shftflg = 1;     % use opt bc shift 1 = yes, 0 = no
%
...
for trl = 1:ntrl
  ...
  for i = 1:ncnt
    if isnan(rbcf(i)) == true
      rbcf(i) = 0;
      nancnt = nancnt + 1;
    end
    if isnan(rbcn(i)) == true 
      rbcn(i) = 0;
      nancnt = nancnt + 1;      
    end
  end    
