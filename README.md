...
- MATLAB with the Image Processing and Statistics & Machine Learning toolboxes (and the Deep Learning
  Toolbox for the twin CNN in `cnn/`).
...
├── cnn/                      twin/Siamese-CNN comparison (learned; needs Deep Learning Toolbox)
...
## Twin CNN (learned comparison)

`cnn/` holds a twin/Siamese convolutional network that plays the same game as the Bayesian model,
...
far a learned model transfers. `train_cnn.m` is the entry point; `cnn/cnn_plan.md` tracks the design,
results, and open to-dos. It needs the Deep Learning Toolbox. The trained network ships as
`data/models/net_on_nat.mat`; the texture test set is generated by `texture_same_diff_patches_cnn.m`
into the git-ignored `data/stimuli/textures/`.

The CNN and the Bayesian pipeline share one ingestion path: both read parameters from `config.m` and
...
the CNN samples fresh pairs on the fly).
