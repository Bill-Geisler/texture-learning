function [groups, ngrps] = merge_similar_regions(groups, ngrps, group_counts, groups2d, n_patches, content_sim, merge_criterion)
...
%       group_counts, groups2d, n_patches, content_sim, merge_criterion)
%
...
%     groups, ngrps - updated membership and group count.
    ...
    for k = 1:ngrps
        for i = 1:n_patches
            for j = 1:n_patches
                if groups2d(i, j) == k
                    if j < n_patches && groups2d(i, j+1) ~= k, touching(k, groups2d(i, j+1)) = 1; end
                    if j > 1         && groups2d(i, j-1) ~= k, touching(k, groups2d(i, j-1)) = 1; end
                    if i < n_patches && groups2d(i+1, j) ~= k, touching(k, groups2d(i+1, j)) = 1; end
